...
install plane (root): root broker -> embedded xpad-install -> XPad OEM Provider
install plane (ADB): shell broker -> embedded xpad-install -> managed 0044
...
installer Provider and verifies the resulting installer source. An ADB-shell
broker derives the OEM installer UID per device and uses managed 0044; bounded
31317 may only repair a missing or broken 0044 identity.
...
original staging file remains private. ADB-shell mode repairs and re-verifies managed 0044 when needed, then
installs the target through 0044. Its guarded 31317 transaction is used solely
to repair a missing or broken 0044 identity before target installation starts.
The shell path waits up to five minutes for the repaired alias to become healthy.
If Android is still converging,
...
The UI tells users that repair polling can take up to about five minutes. A pending
repair exposes a read-only Recheck action that only runs `znxrun status`. Exit 75 is
reported as “ordinary reboot required” rather
