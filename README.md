...
- embedded installation engine: [`xpad-installer`](https://github.com/yoyicue/xpad-installer)
  v0.2.6, hash-locked inside the APK and extracted only for one broker operation;
...
`xpad-install` v0.2.6 ELF recorded in `third_party/xpad-installer.lock`. The
...
broker, which extracts and verifies its embedded `xpad-install` v0.2.6, copies
...
broken 0044 identity before target installation starts. Version 0.2.6 waits up to
60 seconds for the repaired alias to become fully healthy before deciding whether the repair
...
The build first resolves the public xpad-installer v0.2.6 Release, verifies its
...
xpad-installer v0.2.6 is GPL-3.0-only; its full license is included in the APK
